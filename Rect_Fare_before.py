...
def f(x):
    ...
    if x[0] > 0 and x[0] <= 0.5:
        ...
        WalkDist_l1 = 8 * AreaSide_s * x[0] / 3
        WalkDist_l2 = 4 * AreaSide_s * x[0] / 3


    else:
        FixedArea_a1 = 1 - 2 * (1 - x[0]) ** 2
        WalkDist_l1 = (6 * AreaSide_s - 8 * (1 - x[0]) ** 2 * (2 * AreaSide_s * x[0] + AreaSide_s)) / (3 - 6 * (1 - x[0]) ** 2)
        WalkDist_l2 = (3 * AreaSide_s - 4 * (1 - x[0]) ** 2 * (2 * AreaSide_s * x[0] + AreaSide_s)) / (3 - 6 * (1 - x[0]) ** 2)
    ...
    # 车辆行驶距离
    DistFixed_d1 = 2 * AreaLength_D / x[2]
    DistFlex_d2 = 2 * AreaLength_D / x[3] + 4 * AreaLength_D * AreaSide_s ** 2 * Passenger * FlexArea_a2 / 3
